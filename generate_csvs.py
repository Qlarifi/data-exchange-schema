...
def create_aggregated_csv_files_for_currency(scenario_dir, currency_name):
    """Create aggregated CSV files for a specific currency (EUR or USD)"""
    
    # Collect all events from scenarios in this directory, grouped by event type
    events_by_type = defaultdict(list)
    
    for i in range(1, 10):  # scenarios 1-9
        scenario_file = os.path.join(scenario_dir, f'scenario{i}.json')
    ...
    # Create aggregated CSV files in the same directory
    for event_type, all_events in events_by_type.items():
        csv_filename = f"all_{event_type}.csv"
        ...
        print(f"Created aggregated {currency_name}: {csv_path}")
...
def main():
    ...
    # Create aggregated CSV files for each currency
    print("\nCreating aggregated CSV files...")
    create_aggregated_csv_files_for_currency(eur_dir, "EUR")
    create_aggregated_csv_files_for_currency(usd_dir, "USD")
